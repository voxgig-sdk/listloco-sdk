# Listloco SDK test runner

from __future__ import annotations
import os
import json

from listloco_sdk.utility.voxgig_struct import voxgig_struct as vs


class ListlocoTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ListlocoTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ListlocoTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ListlocoTestRunner.getenv("LISTLOCO_TEST_LIVE")
        override = ListlocoTestRunner.getenv("LISTLOCO_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ListlocoTestRunner.getenv(key)
        ...
        explain = ListlocoTestRunner.getenv("LISTLOCO_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["LISTLOCO_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if ListlocoTestRunner._test_control is not None:
            return ListlocoTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ListlocoTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ListlocoTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ListlocoTestRunner.load_test_control()
...
def load_env_local():
    ListlocoTestRunner.load_env_local()


def env_override(m):
    return ListlocoTestRunner.env_override(m)


def entity_data(v):
    return ListlocoTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ListlocoTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ListlocoTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ListlocoTestRunner.load_test_control()


def live_client_options():
    return ListlocoTestRunner.live_client_options()


def live_delay_ms():
    return ListlocoTestRunner.live_delay_ms()
