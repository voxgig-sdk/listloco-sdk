...
const TEST_SEED = { "entity": { "localize": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+ListlocoSDK\b/.test(code) ||
        /\bListlocoSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+ListlocoSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/ListlocoSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
