// Command Listloco-cli is a CLI + REPL for the Listloco SDK,
...
const prompt = "listloco"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "localize"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      LISTLOCO_APIKEY (api key), LISTLOCO_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: LISTLOCO_APIKEY carries the API key and
	// LISTLOCO_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("LISTLOCO_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("LISTLOCO_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewListlocoSDK(opts)
