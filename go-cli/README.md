...
boru-driven command-line client **and** interactive REPL for the Listloco
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/listloco-cli)
make build
...
LISTLOCO_BASE=https://api.example.com ./listloco-cli --help
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/listloco-cli
   ```
...
Run with no arguments to open a REPL (prompt `listloco>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `localize`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/listloco-cli`. |
...
(`./listloco-cli <expr>`) and interactively (the REPL), and expressions compose the
