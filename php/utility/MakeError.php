...
class ListlocoMakeError
{
    public static function call(?ListlocoContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new ListlocoContext([], null);
        }
        $op = $ctx->op ?? new ListlocoOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new ListlocoResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof ListlocoError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new ListlocoError('', $msg, $ctx);
        ...
        if ($err instanceof ListlocoError) {
