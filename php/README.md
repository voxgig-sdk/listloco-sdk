...
The PHP SDK for the Listloco API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->Localize()` — with named operations (`create`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
- Releases: [https://github.com/voxgig-sdk/listloco-sdk/releases](https://github.com/voxgig-sdk/listloco-sdk/releases)
...
```php
<?php
require_once 'listloco_sdk.php';

$client = new ListlocoSDK([
    "apikey" => getenv("LISTLOCO_APIKEY"),
]);
```
...
```
LISTLOCO_TEST_LIVE=TRUE
LISTLOCO_APIKEY=<your-key>
```
...
```php
require_once 'listloco_sdk.php';
$client = new ListlocoSDK($options);
```
...
```php
$client = ListlocoSDK::test($testopts, $sdkopts);
```
...
| `Localize` | `($data): LocalizeEntity` | Create a Localize entity instance. |
...
Create an instance: `$localize = $client->Localize();`
...
| --- | --- | --- |
| `dictionary` | `array` | Custom translation dictionary mapping source terms to target translations |
| `gates` | `array` | Deterministic quality gate results |
| `glossary` | `array` | Customer glossary for enforcing brand terms and model numbers |
| `listing` | `array` | Product listing information to be localized |
| `localized` | `array` | Localized listing content |
...
| `violations` | `array` | List of compliance violations if any gate failed |
...
```
php/
├── listloco_sdk.php          -- Main SDK class
...
The main class (`listloco_sdk.php`) exports the SDK class
