...
class ListlocoSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = ListlocoHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = ListlocoHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!ListlocoFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, ListlocoFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return ListlocoUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = ListlocoHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = ListlocoHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = ListlocoHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new ListlocoError($op . "_allow",
                "ListlocoSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = ListlocoHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new ListlocoError("graphql_error",
                "ListlocoSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    private $_localize = null;

    // Canonical facade: $client->Localize()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->localize()
    // resolves here too.
    public function Localize($data = null)
    {
        require_once __DIR__ . '/entity/localize_entity.php';
        if ($data === null) {
            if ($this->_localize === null) {
                $this->_localize = new LocalizeEntity($this, null);
            }
            return $this->_localize;
        }
        return new LocalizeEntity($this, $data);
    }
