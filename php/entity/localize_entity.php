...
class LocalizeEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new LocalizeEntity($this->_client, $opts);
    }

    /**
     * @param Localize|array $args Localize data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = ListlocoHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Localize|array The current Localize data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Localize fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = ListlocoHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Localize fields).
     */
    ...
    /**
     * Create a new Localize.
     *
     * @param LocalizeCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed LocalizeCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Localize|array The created Localize as an assoc-array at the
     *   SDK boundary; throws ListlocoError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                if ($ctx->result->resdata) {
                    $this->_data = ListlocoHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
