...
class FtCtrl extends ListlocoControl
{
...
class FtClient
{
    ...
    private ListlocoUtility $utility;

    public function __construct(ListlocoUtility $utility, array $options)
    {
    ...
    public function get_utility(): ListlocoUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ListlocoContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ListlocoContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ListlocoContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ListlocoConfig::make_feature($name);
    ...
    private static function buildUrl(ListlocoSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ListlocoError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ListlocoResponse($fetched);
        }
    ...
    private function fail(ListlocoContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ListlocoContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
