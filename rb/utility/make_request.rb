...
module ListlocoUtilities
  MakeRequest = ->(ctx) {
    ...
    if fetch_err
      ...
      response = ListlocoResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ListlocoResponse.new(fetched)
    else
