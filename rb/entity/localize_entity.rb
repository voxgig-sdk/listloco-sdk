...
class LocalizeEntity
  ...
  def make
    opts = @_entopts.dup
    LocalizeEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = ListlocoHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Localize, Hash] the current Localize data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = ListlocoHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Localize fields)
  def match_get
  ...
  #
  # @param reqdata [LocalizeCreateData, Hash, nil] body data
  # @param ctrl [Object, nil] optional per-call control
  # @return [Localize, Hash] the created Localize; raises ListlocoError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = ListlocoHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
