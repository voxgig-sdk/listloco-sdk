...
[MCP](https://modelcontextprotocol.io) server exposing the Listloco SDK as
two agent tools — `listloco_list` and `listloco_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/listloco-mcp)
make build

# 2. Provide credentials via the environment
export LISTLOCO_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user listloco \
  -- /absolute/path/to/listloco-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./listloco-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// listloco_list: first page of records
{ "entity": "localize" }
{ "entity": "localize", "query": { } }

// listloco_load: one record by id
{ "entity": "localize", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/listloco-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user listloco \
     -- "$PWD"/dist/*/listloco-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `listloco_list` and `listloco_load` tools now appear
   in new sessions. Ask the agent to *"list localize using listloco"*
   and it calls `listloco_list` with `{"entity":"localize"}`.
...
```sh
export LISTLOCO_APIKEY=sk_live_xxx            # API key
export LISTLOCO_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `listloco_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `listloco_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `LISTLOCO_APIKEY` | API key sent with every request. |
| `LISTLOCO_BASE` | Optional override of the API base URL. |
...
```sh
./listloco-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"listloco_load","arguments":{"entity":"localize","query":{"id":1}}}}'
```
