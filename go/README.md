...
The Golang SDK for the Listloco API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.
...
## Install
```bash
go get github.com/voxgig-sdk/listloco-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/listloco-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/listloco-sdk/go=../listloco-sdk/go
```
...
func main() {
    client := sdk.NewListlocoSDK(map[string]any{
        "apikey": os.Getenv("LISTLOCO_APIKEY"),
    })
...
fmt.Println(localize) // the returned mock data
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *ListlocoSDK
```
...
| `Localize` | `(data map[string]any) ListlocoEntity` | Create a Localize entity instance. |

### Entity interface (ListlocoEntity)

All entities implement the `ListlocoEntity` interface.
...
slice):
    ...
    localize, err := client.Localize(nil).Create(map[string]any{/* fields */}, nil)
    if err != nil { /* handle */ }
    // localize is the returned record
...
Create an instance: `localize := client.Localize(nil)`
...
```
github.com/voxgig-sdk/listloco-sdk/go/
├── listloco.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/listloco-sdk/go`) re-exports everything needed
