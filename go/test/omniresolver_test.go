...
import (
	...
	omni "github.com/voxgig-sdk/listloco-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ListlocoError); is {
				out := map[string]any{
					"name":    "ListlocoError",
...
func omniCtx(arg any, client *sdk.ListlocoSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ListlocoSDK); is {
			client = live
