...
The Lua SDK for the Listloco API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:Localize()` — each with the same small set of operations (`create`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/listloco-sdk/releases)),
...
Create an instance: `local localize = client:Localize(nil)`
...
```
lua/
├── listloco_sdk.lua    -- Main SDK module
...
The main module (`listloco_sdk`) exports the SDK constructor
