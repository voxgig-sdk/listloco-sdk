...
local LocalizeEntity = {}
LocalizeEntity.__index = LocalizeEntity


function LocalizeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LocalizeEntity)
  self._name = "localize"
  self._client = client
...
function LocalizeEntity:get_name()
  return self._name
end


function LocalizeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LocalizeEntity.new(self._client, opts)
end
...
function LocalizeEntity:mark_deleted()
  self._deleted = true
end


function LocalizeEntity:deleted()
  return true == self._deleted
end


function LocalizeEntity:data_set(args)
  if args ~= nil then
...
function LocalizeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LocalizeEntity:match_set(args)
  if args ~= nil then
...
function LocalizeEntity:match_get()
...
function LocalizeEntity:stream(action, args, callopts)
...
---@param reqdata LocalizeCreateData
---@param ctrl? table
---@return Localize
---@return string? err
function LocalizeEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function LocalizeEntity:_run_op(ctx, post_done)
  local utility = self._utility
